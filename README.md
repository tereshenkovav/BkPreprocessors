...
## Инструкция по сборке препроцессора для Бейсика

Версия для Delphi компилируется через открытие проекта `BkBasicPreprocessor\BkBasicPreprocessor.dproj`.
Для сборки версии FreePascal, нужно запустить пакетный файл из каталога `build`.
Для компиляции проекта нужно добавить в каталог `BkBasicPreprocessor` файл версии `Version.pas`.
